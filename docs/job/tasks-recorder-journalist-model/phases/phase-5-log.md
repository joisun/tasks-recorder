...
| 2026-08-21 | feature commit reconciliation | done | Phase 1–5 分别提交为 `1875154`、`96b14ff`、`5e83ffb`、`d7ddfee`、`b40a200`；commit 前 fresh gate 为 280/280 tests、85-file syntax、UI/adapters/release builds 与 diff check 全绿。 |
| 2026-08-21 | main CI timezone regression | failed | GitHub Actions run `32473578515` 在 UTC 环境为 279/280；唯一失败是 Timeline test 将 UTC instant 硬编码为 Asia/Shanghai local-calendar 断言，release tag 未创建。 |
| 2026-08-21 | main CI timezone regression | fixed | [MDN Date reference](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date) 确认 `getDate`/`setDate`/`setHours` 使用 host local timezone；将 test fixture 改为显式 local-calendar constructor。`TZ=UTC` 与 `TZ=Asia/Shanghai` focused tests 均 19/19。 |
