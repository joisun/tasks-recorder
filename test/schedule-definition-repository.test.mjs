...
test('Fast mode survives disk reload and unrelated edits and can return to inherited configuration', async (t) => {
  const { root, repository } = await fixture(t)
  const id = randomUUID()
  await writeFile(join(root, 'fast.md'), definition(id))
  await repository.scan()
  let job = await repository.get(id)
  job = await repository.update(id, job.etag, { fast_mode: true })
  assert.equal((await repository.scan()).jobs[0].fast_mode, true)
  job = await repository.update(id, job.etag, { title: 'Keep Fast' })
  assert.equal(job.fast_mode, true)
  job = await repository.update(id, job.etag, { fast_mode: false })
  assert.equal(job.fast_mode, false)
  job = await repository.update(id, job.etag, { fast_mode: null })
  assert.equal(job.fast_mode, undefined)
  assert.doesNotMatch(await readFile(job.source_path, 'utf8'), /fast_mode/)
})
