...
test('installer creates versioned runtime and preserves existing config/database on reinstall', async () => {
  ...
  try {
    ...
    const first = await runInstaller({
      homeDirectory, releaseDirectory, args: ['--version', 'v0.3.3', '--no-start'],
    })
    assert.equal(first.code, 0, first.stderr)
    assert.match(first.stdout, /Installed Tasks Recorder 0\.3\.3/)
    ...
    assert.equal(currentTarget, join(installRoot, 'releases', '0.3.3'))
    assert.match(
...
test('installer replaces an existing current symlink instead of nesting it in the old release', async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      join(installRoot, 'releases', '0.3.3'),
    )
