...
export function executionId({ sessionId, turnKey }) {
  const identity = ['claude', sessionId, turnKey, ''].join('\u0000')
...
async function envelope(input, turnKey, eventType, externalEventId, payload) {
...
export async function sessionStartedEvent(input) {
  const sessionId = requiredString(input?.session_id, 'session_id')
  return envelope(input, null, 'session.started', `claude:session:${sessionId}:started`, {})
}
...
export async function mainExecutionStartedEvent(input, turnKey) {
  ...
  return envelope(
    ...
    'execution.started',
    `claude:execution:${sessionId}:${turnKey}:started`,
    { kind: 'main' },
  )
}
...
export async function toolHeartbeatEvent(input, turnKey) {
  ...
  return envelope(
    ...
    requiredString(turnKey, 'turn_key'),
    'execution.heartbeat',
    `claude:execution:${sessionId}:${turnKey}:tool:${eventKey}`,
    { activity: 'tool_use' },
  )
}
...
export async function mainExecutionStoppedEvent(input, turnKey) {
  ...
  return envelope(
    ...
    requiredString(turnKey, 'turn_key'),
    'execution.stop',
    `claude:execution:${sessionId}:${turnKey}:stopped`,
    { end_reason: 'stopped' },
  )
}
...
export async function sessionEndedEvent(input) {
  ...
  return envelope(
    ...
    null,
    'session.ended',
    `claude:session:${sessionId}:ended`,
